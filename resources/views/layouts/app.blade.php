    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.fetchDashboardMetrics = function() {
                if (!document.getElementById('stat-avg-risk')) return;
                    ...
                    .then(res => {
                        if (res.status === 'success') {
                            ...
                            if (recentAlertsList && res.data.recentAlerts) {
                                ...
                                if (html === '') {
                                    html = `<div class="text-center text-muted py-4"><i class="bi bi-shield-check display-6 d-block mb-2 text-success"></i><p class="small mb-0">No active system alerts recorded.</p></div>`;
                                }
            ...
            window.fetchWeather = function() {
                if (!document.getElementById('stat-weather-stations')) return;
            ...
            window.fetchCurrency = function() {
                if (!document.getElementById('stat-currency-tracked')) return;
            ...
            window.fetchNews = function() {
                if (!document.getElementById('news-articles-grid')) return;
                                            ...
                                            <div class="card card-premium border-0 h-100" style="transition: transform 0.2s;">
                                                ...
                                                <div class="card-body d-flex flex-column">
                                                    ...
                                                    <div class="d-flex align-items-center justify-content-between mt-auto">
                                                        ...
                                                        ${a.source_url ? `<a href="${a.source_url}" target="_blank" rel="noopener noreferrer" class="btn btn-sm btn-outline-primary py-0 px-2 small"><i class="bi bi-box-arrow-up-right me-1"></i>Source</a>` : ''}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
            ...
            window.fetchCountryRisk = function() {
            ...
            // Execute initial fetch calls
            if (document.getElementById('stat-avg-risk')) window.fetchDashboardMetrics();
            if (document.getElementById('stat-weather-stations')) window.fetchWeather();
            if (document.getElementById('stat-currency-tracked')) window.fetchCurrency();
            if (document.getElementById('news-articles-grid')) window.fetchNews();
            if (document.getElementById('country-detail-risk-breakdown-card')) window.fetchCountryRisk();

            // Periodic fallback polling intervals
            if (document.getElementById('stat-avg-risk')) setInterval(window.fetchDashboardMetrics, 60000);
            if (document.getElementById('stat-weather-stations')) setInterval(window.fetchWeather, 30000);
            if (document.getElementById('stat-currency-tracked')) setInterval(window.fetchCurrency, 30000);
            if (document.getElementById('news-articles-grid')) setInterval(window.fetchNews, 60000);
            if (document.getElementById('country-detail-risk-breakdown-card')) setInterval(window.fetchCountryRisk, 60000);
                ...
                try {
                    ...
                    window.Echo.channel('weather-channel')
                        .listen('.WeatherUpdated', (e) => {
                            console.log('📡 Realtime Weather Event:', e);
                            window.fetchWeather();
                            window.fetchDashboardMetrics();
                        });

                    // 2. Currency Channel
                    window.Echo.channel('currency-channel')
                        .listen('.ExchangeRateUpdated', (e) => {
                            console.log('📡 Realtime Currency Event:', e);
                            window.fetchCurrency();
                            window.fetchDashboardMetrics();
                        });

                    // 3. News Channel
                    window.Echo.channel('news-channel')
                        .listen('.NewsUpdated', (e) => {
                            console.log('📡 Realtime News Event:', e);
                            window.fetchNews();
                            window.fetchDashboardMetrics();
                        });

                    // 4. Risk Channel
                    window.Echo.channel('risk-channel')
                        .listen('.RiskScoreUpdated', (e) => {
                            console.log('📡 Realtime Risk Score Event:', e);
                            window.fetchDashboardMetrics();
                            window.fetchCountryRisk();
                        });

                    // 5. Country Channel
                    window.Echo.channel('country-channel')
                        .listen('.CountryUpdated', (e) => {
                            console.log('📡 Realtime Country Event:', e);
                            window.fetchDashboardMetrics();
                        });
